...
vector<uint32_t> SHA512_256::convert_uint32_t_from_string(string message, int message_type) {
...
void SHA512_256::processMessage()
{
	vector<uint64_t> message = this->message;
	vector<uint64_t> hash = this->hash;
	vector<uint64_t> w(80);
	H[0] = 0x22312194FC2BF72C; H[1] = 0x9F555FA3C84C64C2; H[2] = 0x2393B86B6F53B151; H[3] = 0x963877195940EABD; H[4] = 0x96283EE2A88EFFE3;
	H[5] = 0xBE5E1E2553863992; H[6] = 0x2B0199FC2C85B8AA; H[7] = 0x0EB72DDC81C52CA2;
...
void SHA512_256::processMessageChunk(vector<uint64_t>& in)
{
	vector<uint64_t> w(80);
	// Extend the message
...
vector<uint32_t> SHA512_256::getHashMessageByuint32(vector<uint32_t> message, size_t message_size) {
	...
	return vector<uint32_t>(result.begin(), result.begin() + 8);
}


string SHA512_256::getHashMessage(string message, int message_type, int output_type) {

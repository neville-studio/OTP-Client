...
void SNTPClient::transformNTPPackage(BYTE* recvBuf, int recvBufLen, int64_t send, int64_t recv)
{
	...
	this->resultTimestamp = (0LL + server_recv + int64_t(server_send - server_recv) / 2 + (recv - send) / 2) * 1000 + (server_send_sec - server_recv_sec) * 1000LL / (1LL << 32);
	...
	this->last_update = std::chrono::system_clock::now().time_since_epoch().count();

}

int64_t SNTPClient::getSNTPTimestamp(const char* server, uint16_t port, bool enableIpV6)
{
